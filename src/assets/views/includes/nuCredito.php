<section class="nucredito nucredito--disabled">
    <header class="nucredito__header">
        <div class="nucredito__header__back">
            <img class="nucredito__header__back__arrow" src="dist/img/icons/arrow-down.png" alt="left arrow">
            <img src="dist/img/icons/gray/coin.png" alt="gift icon">
        </div>
        <img src="dist/img/icons/gray/search.png" alt="question icon">
    </header>

    <div class="nucredito__slider">
        <div class="nucredito__slider__slide">
            <h2 class="nucredito__slider__slide__title">R$ <span>1.328</span>,40</h2>
            <span class="nucredito__slider__slide__vencimento">Vencimento 25 mai</span>
            <a class="nucredito__slider__slide__pagar" href="#">Pagar</a>
        </div>

        <div class="nucredito__slider__slide">
            <h2 class="nucredito__slider__slide__title">Bloco teste</h2>
        </div>
    </div>

    <ul class="nucredito__list">
        <li class="nucredito__list__item">
            <figure class="nucredito__list__item__imgWrapper">
                <img src="dist/img/icons/gray/travel.png" alt="Handbag icon">
            </figure>
            <div class="nucredito__list__item__info">
                <h3>Viagem</h3>
                <span class="nucredito__list__item__info__title">Brasil Airlines</span>
                <span class="nucredito__list__item__info__description" >Apagar R$ 268,35 com 26.836pts</span>
            </div>
            <div class="nucredito__list__item__date">
                <span>Domingo</span>
            </div>
        </li>
        <li class="nucredito__list__item">
            <figure class="nucredito__list__item__imgWrapper">
                <img src="dist/img/icons/gray/travel.png" alt="Handbag icon">
            </figure>
            <div class="nucredito__list__item__info">
                <h3>Viagem</h3>
                <span class="nucredito__list__item__info__title">Brasil Airlines</span>
                <span class="nucredito__list__item__info__description" >Apagar R$ 268,35 com 26.836pts</span>
            </div>
            <div class="nucredito__list__item__date">
                <span>Domingo</span>
            </div>
        </li>
        <li class="nucredito__list__item">
            <figure class="nucredito__list__item__imgWrapper">
                <img src="dist/img/icons/gray/travel.png" alt="Handbag icon">
            </figure>
            <div class="nucredito__list__item__info">
                <h3>Viagem</h3>
                <span class="nucredito__list__item__info__title">Brasil Airlines</span>
                <span class="nucredito__list__item__info__description" >Apagar R$ 268,35 com 26.836pts</span>
            </div>
            <div class="nucredito__list__item__date">
                <span>Domingo</span>
            </div>
        </li>
        <li class="nucredito__list__item">
            <figure class="nucredito__list__item__imgWrapper">
                <img src="dist/img/icons/gray/travel.png" alt="Handbag icon">
            </figure>
            <div class="nucredito__list__item__info">
                <h3>Viagem</h3>
                <span class="nucredito__list__item__info__title">Brasil Airlines</span>
                <span class="nucredito__list__item__info__description" >Apagar R$ 268,35 com 26.836pts</span>
            </div>
            <div class="nucredito__list__item__date">
                <span>Domingo</span>
            </div>
        </li>
        <li class="nucredito__list__item">
            <figure class="nucredito__list__item__imgWrapper">
                <img src="dist/img/icons/gray/travel.png" alt="Handbag icon">
            </figure>
            <div class="nucredito__list__item__info">
                <h3>Viagem</h3>
                <span class="nucredito__list__item__info__title">Brasil Airlines</span>
                <span class="nucredito__list__item__info__description" >Apagar R$ 268,35 com 26.836pts</span>
            </div>
            <div class="nucredito__list__item__date">
                <span>Domingo</span>
            </div>
        </li>
    </ul>

    <div class="nucredito__scroll">
        <div class="nucredito__scroll--yellow"></div>
        <div class="nucredito__scroll--green"></div>
        <div class="nucredito__scroll--red"></div>
    </div>
</section>